...
CREATE TABLE IF NOT EXISTS public.users (
    ...
    is_admin BOOLEAN DEFAULT FALSE, -- Legacy field, not used with new super_admin table
...
-- =============================================================================
-- EMAIL_TEMPLATES TABLE (For Resend email notifications)
-- =============================================================================
...
-- Default: username = 'admin', password = 'VestedAdmin2024!'
INSERT INTO public.super_admin (username, password_hash, full_name)
VALUES (
    'admin', 
    '$2a$10$YourHashedPasswordHere', -- Replace with bcrypt hash of 'VestedAdmin2024!'
    'Super Administrator'
)
...
INSERT INTO public.traders (name, bio, total_profit_loss, profit_loss_percentage, followers_count, performance_data) VALUES
    ...
    ('David Kim', 'Altcoin expert with a focus on emerging blockchain technologies.', 156000, 87, 654, '[{"date": "2024-02-20", "value": 100}, {"date": "2024-02-21", "value": 99}, {"date": "2024-02-22", "value": 101}]'),
    ('Lisa Wang', 'Quantitative trader using algorithmic strategies for consistent returns.', 312000, 203, 2156, '[{"date": "2024-02-20", "value": 100}, {"date": "2024-02-21", "value": 104}, {"date": "2024-02-22", "value": 107}]'),
    ('James Wilson', 'Long-term investor with a focus on fundamental analysis.', 78000, 42, 423, '[{"date": "2024-02-20", "value": 100}, {"date": "2024-02-21", "value": 101}, {"date": "2024-02-22", "value": 100}]')
ON CONFLICT DO NOTHING;

-- Insert email templates
INSERT INTO public.email_templates (name, subject, body, variables) VALUES
    ('deposit_received', 
     'Deposit Request Received - Vested', 
     'Hello {{full_name}},\n\nWe have received your deposit request of {{amount}} {{crypto_symbol}}.\n\nOur team will review and process it shortly.\n\nThank you,\nVested Team',
     ARRAY['full_name', 'amount', 'crypto_symbol']),
    
    ('deposit_approved', 
     'Deposit Approved - Vested', 
     'Hello {{full_name}},\n\nYour deposit of {{amount}} {{crypto_symbol}} has been approved and added to your balance.\n\nThank you,\nVested Team',
     ARRAY['full_name', 'amount', 'crypto_symbol']),
    
    ('withdrawal_received', 
     'Withdrawal Request Received - Vested', 
     'Hello {{full_name}},\n\nWe have received your withdrawal request of {{amount}} {{crypto_symbol}}.\n\nOur team will review and process it shortly.\n\nThank you,\nVested Team',
     ARRAY['full_name', 'amount', 'crypto_symbol']),
    
    ('withdrawal_approved', 
     'Withdrawal Approved - Vested', 
     'Hello {{full_name}},\n\nYour withdrawal of {{amount}} {{crypto_symbol}} has been approved and sent to your address.\n\nThank you,\nVested Team',
     ARRAY['full_name', 'amount', 'crypto_symbol'])
...
BEGIN
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM public.traders WHERE is_active = TRUE),
        (SELECT COUNT(*) FROM public.cryptos WHERE is_active = TRUE);
END;
$$ LANGUAGE plpgsql;

-- Function to update super admin credentials
CREATE OR REPLACE FUNCTION update_super_admin_credentials(
...
BEGIN
    ...
    -- For now, simple text comparison for demo
    IF v_stored_hash != p_current_password THEN
        RETURN FALSE;
    END IF;
